...
float global_offset_x = 2.0;
float global_offset_y = -2.0;
...
{
    ros::init(argc, argv, "control_iris_3");
    setlocale(LC_ALL, "");
    ros::NodeHandle nh;
    std::cout << "start" << std::endl;

    // 初始化订阅者和服务客户端
    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("/iris_3/mavros/state", 10, state_cb);
    ros::Publisher local_pos_pub = nh.advertise<mavros_msgs::PositionTarget>("/iris_3/mavros/setpoint_raw/local", 1);
    ros::Publisher pubMarker = nh.advertise<visualization_msgs::Marker>("/track_drone_point", 5);
    pubMarkerPointer = &pubMarker;
    ros::Subscriber rc_sub = nh.subscribe<mavros_msgs::RCIn>("/iris_3/mavros/rc/in", 10, rc_cb);
    ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>("/iris_3/mavros/cmd/arming");
    ros::ServiceClient set_mode_client = nh.serviceClient<mavros_msgs::SetMode>("/iris_3/mavros/set_mode");
    ros::Subscriber twist_sub = nh.subscribe<quadrotor_msgs::PositionCommand>("/planning/pos_cmd_4", 10, twist_cb);
    ros::Subscriber target_sub = nh.subscribe<geometry_msgs::PoseStamped>("move_base_simple/goal", 10, target_cb);
    ros::Subscriber position_sub = nh.subscribe<nav_msgs::Odometry>("/iris_3/mavros/local_position/odom", 10, position_cb);
    ros::Rate rate(50.0);
